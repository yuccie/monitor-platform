...
const pm = (function() {
  ...
  // 获取所有关于performance里的数据
  pMonitor.getPerformance = () => {
  ...
  pMonitor._request = (url, data, type = 'POST') => {
  ...
  pMonitor.logPackage = () => {
    ...
    const data = pMonitor.getPerformance();
    // 无法操作只读属性
    // data.serverTiming = JSON.stringify(data.serverTiming);

    // 上报页面加载时间
    pMonitor._request(url, data, method);
    // 上传超时数据列表
